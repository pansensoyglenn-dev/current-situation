...
// Builds a standalone HTML page with proper Open Graph / Twitter Card tags
// (title, description, image) for one article. Facebook's crawler doesn't
// run JavaScript, so this page — not the SPA hash route — is what needs to
// carry the preview image. It redirects real visitors into the app.
// canonicalUrl (Vercel) is what the page *declares* as authoritative for SEO;
// liveUrl (Cloudflare/Workers) is where visitors are actually redirected —
// the two can differ since this repo is mirrored to more than one host.
...
async function deployArticleToGitHub(article) {
  ...
  try {
    ...
    // Fall back to the section's cover photo if the article itself has no photo.
    const imageAbsUrl = photoPath
      ? `${SITE_URL}${photoPath}`
      : (cat ? `${SITE_URL}covers/${cat.slug}.jpg` : null);

    // liveUrl is where the page is actually deployed and reachable (Cloudflare/Workers,
    // driven by GitHub auto-deploy) — used for real share links people click.
    // canonicalUrl is the SEO-authoritative address (Vercel) declared inside the page.
...
// Posts a link to the article's live share page (which carries proper OG tags,
// so Facebook renders a real title/description/image preview automatically).
...
// SITE_URL: where GitHub auto-deploy actually pushes content, and what real
// visitors and share links point to. CANONICAL_URL: the SEO-declared
// "authoritative" address for <link rel="canonical"> and og:url tags.
// Both point at Vercel — confirmed live, in sync, and the intended canonical.
...
// ---------- Backup & Restore ----------
// Downloads a JSON file (articles, photos, tags, GitHub/AI settings) plus a
// separate file per video, matched up by filename. Restore reverses this:
// pick the JSON back in, optionally add the matching video files, and it
// rebuilds everything in IndexedDB.
  ...
  try {
    ...
    for (const a of articles) {
      if (a.video && a.video.startsWith('blobref:')) {
        ...
        } catch (e) { /* fall back to default ext */ }
