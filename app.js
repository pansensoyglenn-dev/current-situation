...
// ---------- Static article page (SEO) ----------
// Renders a complete, server-side-visible HTML page for one article.
// No redirects — Google must be able to see the article's full content
// in the raw HTML for it to be indexed. A visible link at the bottom
// lets real visitors open the interactive SPA version.
function articleStaticHtml(article, cat, canonicalUrl, imageAbsUrl) {
  ...
  const tagsHtml = (article.tags && article.tags.length)
    ? `<div class="tag-row">${article.tags.map(t =>
  ...
  const heroHtml = imageAbsUrl
    ? `<img class="article-photo" src="${imageAbsUrl}" alt="${title}" loading="eager">`
    : '';
  ...
  const jsonLd = {
    ...
    description: article.summary || shareSnippet(article),
    datePublished: article.date,
    author: { '@type': 'Person', name: 'Glenn Pansensoy' },
    publisher: { '@type': 'Organization', name: 'The Commonplace' },
    mainEntityOfPage: canonicalUrl
  };
  if (imageAbsUrl) jsonLd.image = imageAbsUrl;
...
<head>
  ...
  <meta property="article:author" content="Glenn Pansensoy">${imageTag}
  <meta name="twitter:title" content="${title}">
  <meta name="twitter:description" content="${desc}">

  <link rel="icon" type="image/svg+xml" href="${SITE_URL}current-situation/favicon.svg">
  ...
  <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-6428799122405621" crossorigin="anonymous"></script>

  <script type="application/ld+json">${JSON.stringify(jsonLd)}</script>
</head>
<body>
  <div class="legal-page section-landing">
    <a class="back-link" href="${SITE_URL}">← The Commonplace</a>
    <div class="article-header" style="margin-top:1rem;">
      ${cat ? `<span class="category-tag" style="color:${cat.color};border-color:${cat.color};">${catName}</span>` : ''}
    ...
    <div class="ad-slot page-col" style="margin-top:2rem;">
    ...
    <div class="btn-row" style="margin-top:2rem;">
      <a class="btn-primary" href="${spaUrl}" style="text-decoration:none;display:inline-block;">Open in The Commonplace →</a>
    </div>
  </div>
</body>
</html>
...
// ---------- Sitemap generator ----------
function generateSitemap(allArticles, baseUrl) {
  ...
  const staticPages = [
    ...
    { path: 'cookies.html', priority: '0.5', freq: 'monthly' },
    { path: 'accessibility.html', priority: '0.5', freq: 'monthly' },
  ...
  for (const a of allArticles) {
    ...
    urls.push(`  <url>
    <loc>${baseUrl}${folder}/${a.date}-${slug}.html</loc>
...
async function deployArticleToGitHub(article) {
  ...
  try {
    ...
    // Keep sitemap.xml and robots.txt in sync with every deploy.
    try {
      const sitemapXml = generateSitemap(articles, SITE_URL);
      await githubPutFile('sitemap.xml', b64EncodeUnicode(sitemapXml), 'Update sitemap');
      const robotsTxt = generateRobotsTxt(SITE_URL);
      await githubPutFile('robots.txt', b64EncodeUnicode(robotsTxt), 'Update robots.txt');
    } catch (e) {
      console.warn('Sitemap/robots update failed (non-fatal):', e);
    }
    ...
    if (idx !== -1) {
      ...
      maybeAutoPostFacebook(articles[idx]);
    }
...
async function loadFbSettings() {
  ...
  document.getElementById('fb-page-id').value = fbSettings.pageId || '';
  document.getElementById('fb-access-token').value = fbSettings.accessToken || '';
  document.getElementById('fb-autopost').checked = !!fbSettings.autoPost;
}
...
async function saveFbSettings() {
  ...
  try {
    ...
    document.getElementById('fb-status').textContent = fbSettings.autoPost
...
async function postToFacebook(article) {
  ...
  const link = article.pageUrl || articleShareUrl(article);
  if (!article.pageUrl) {
  ...
  try {
    ...
    for (const a of articles) {
      if (a.video && a.video.startsWith('blobref:')) {
        ...
        try {
          const res = await window.storage.get(key);
          if (res && res.value && res.value.type) {
            const m = res.value.type.match(/\/([a-z0-9]+)/i);
            if (m) ext = m[1];
          }
        } catch (e) { }
        videoManifest.push({ articleId: a.id, key, filename: `commonplace-video-${key}.${ext}` });
      }
    }
    ...
    downloadBlob(new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' }), `commonplace-backup-${todayISO()}.json`);

    for (const v of videoManifest) {
      try {
        const res = await window.storage.get(v.key);
        if (res && res.value) {
          let blob = null;
          if (res.value.blob instanceof Blob) blob = res.value.blob;
          else if (res.value.buffer) blob = new Blob([res.value.buffer], { type: res.value.type || 'video/mp4' });
          if (blob) downloadBlob(blob, v.filename);
        }
      } catch (e) { console.warn('Could not include video in backup:', v.key, e); }
    }

    statusEl.textContent = videoManifest.length
      ? `✅ Downloaded backup + ${videoManifest.length} video file(s). Keep them together — you'll need both to restore.`
      : '✅ Backup downloaded.';
...
async function restoreFromBackup() {
  ...
  const videoInput = document.getElementById('restore-video-input');
  ...
  if (!confirm('Restoring will replace all articles currently on this device with the ones in the backup file. Continue?')) return;
  ...
  try {
    ...
    if (!data || !Array.isArray(data.articles)) throw new Error('That file doesn\'t look like a Commonplace backup');

    const videoFiles = Array.from((videoInput && videoInput.files) || []);
    const videoManifest = Array.isArray(data.videos) ? data.videos : [];
    let restoredVideos = 0;
    const missingVideos = [];
    for (const v of videoManifest) {
      const file = videoFiles.find(f => f.name === v.filename);
      if (file) {
        await window.storage.set(v.key, { blob: file, type: file.type || 'video/mp4' });
        restoredVideos++;
      } else {
        missingVideos.push(v.filename);
      }
    }

    articles = data.articles;
    nextId = data.nextId || (articles.length ? Math.max(...articles.map(a => a.id)) + 1 : 1);
    await saveArticlesToStorage();
    ...
    let msg = `✅ Restored ${articles.length} article(s).`;
    if (videoManifest.length) msg += ` ${restoredVideos}/${videoManifest.length} video(s) restored.`;
    if (missingVideos.length) msg += ` Missing video file(s): ${missingVideos.join(', ')}`;
