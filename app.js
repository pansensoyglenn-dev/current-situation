...
async function storeVideoFile(file) {
  ...
  // Store the Blob directly — avoids reading the whole file into a JS ArrayBuffer,
  // which is what previously risked failing/crashing on large files on memory-constrained devices.
    ...
    try {
      ...
      if (res && res.value) {
        ...
        if (res.value.blob instanceof Blob) {
          ...
          // backward compatibility with videos saved before this fix
...
// Sanitizes user-pasted raw HTML before it's ever inserted into the page.
// Strips scripts/embeds/forms entirely, and strips event-handler attributes
// and javascript: URLs from whatever tags remain.
...
function renderCategoryNav() {
  ...
  spineOut.innerHTML = CATEGORIES.map(c => `
    <a class="spine-tab" data-view="articles" data-category="${c.slug}" title="${escapeHtml(c.description)}">
...
document.getElementById('entry-video').addEventListener('change', async (e) => {
  ...
  if (navigator.storage && navigator.storage.estimate) {
    ...
    } catch (err) { /* estimate unsupported/failed — proceed, saveEntry() will still catch a real failure */ }
  }
...
openDB().then(async () => {
  ...
  if (hashMatch) {
    navigateTo('article', { id: parseInt(hashMatch[1], 10) });
  } else {
    navigateTo('home');
  }
