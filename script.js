// ---------- The Commonplace ----------
// A personal article archive, organized into fixed sections. Fully offline (IndexedDB) —
// nothing here talks to a server.

const CATEGORIES = [
  { slug: 'history', name: 'History', color: 'var(--c-history)' },
  { slug: 'philosophy', name: 'Philosophy', color: 'var(--c-philosophy)' },
  { slug: 'economy', name: 'Economy', color: 'var(--c-economy)' },
  { slug: 'programming', name: 'Programming', color: 'var(--c-programming)' },
  { slug: 'culture', name: 'Culture', color: 'var(--c-culture)' },
  { slug: 'local-politics', name: 'Local Politics', color: 'var(--c-local-politics)' },
  { slug: 'global-politics', name: 'Global Politics', color: 'var(--c-global-politics)' },
  { slug: 'weather', name: 'Weather', color: 'var(--c-weather)' }
];
...
let pendingVideoFile = null; // raw File, converted to ArrayBuffer on save
let toastTimer = null;
...
// ---------- Media (photo as base64, video as ArrayBuffer-backed Blob) ----------
// Photos stay small enough that base64-in-JSON is fine. Videos are stored as their own
// ArrayBuffer records (not raw Blobs — Safari/WebKit has known bugs storing Blobs directly
// in IndexedDB that can silently corrupt or empty them) and referenced as "blobref:<key>".
const MAX_VIDEO_BYTES = 500 * 1024 * 1024; // ~500MB — bounded by device storage & browser quota from here
...
async function deleteVideoBlobIfAny(value) {
  if (value && value.startsWith('blobref:')) {
    ...
    try { await window.storage.delete(key); } catch (e) { /* best-effort */ }
  }
}
  ...
  out.innerHTML = list.map(a => {
    ...
    return `
      <div class="article-row" data-id="${a.id}">
        ...
        ${c ? `<span class="category-tag" style="color:${c.color};border-color:${c.color};background:color-mix(in srgb, ${c.color} 14%, transparent);">${c.name}</span>` : ''}
      </div>`;
  }).join('');
...
function openArticle(id) {
  ...
  document.getElementById('article-category-tag').style.cssText = c ? `color:${c.color};border-color:${c.color};background:color-mix(in srgb, ${c.color} 14%, transparent);` : '';
...
// ---------- Share ----------
// A web app can't post directly into Facebook/Instagram/etc. — those need a registered
// developer app and OAuth review. What actually works: the device's native share sheet
// (Web Share API), which lets the person pick Facebook, Instagram, Messenger, WhatsApp,
// whatever's installed — same as sharing a photo from their gallery. Desktop/unsupported
// browsers fall back to downloading the media plus copying the caption.
...
document.getElementById('entry-video').addEventListener('change', (e) => {
  ...
  if (file.size > MAX_VIDEO_BYTES) {
    showToast(`⚠️ That video is too large (limit ~${Math.round(MAX_VIDEO_BYTES / 1024 / 1024)}MB)`, 'error', 5000);
    e.target.value = '';
    return;
  }
