function todayISO() {
      ...
      pageItems.forEach(r => {
        html += `
          <tr>
            ...
            <td>${r.date}</td>
            <td><span class="tag">${r.plantation_type}</span>${r.section ? ` <span class="tag" style="color:var(--fuchsia-bright);border-color:var(--fuchsia-deep);">${r.section}</span>` : ""}</td>
    ...
    function renderNetSalesInputs() {
      ...
      out.innerHTML = PLANTATION_TYPES.map(type => {
        ...
        <div class="field">
          ...
          ${cs.count > 0 ? `<div class="net-sales-readout">🛒 Auto from ${cs.count} customer sale(s): <b>${peso(cs.total)}</b> (${cs.quantity} kg) <button type="button" class="btn-ghost btn-sm" style="min-height:22px;padding:2px 8px;font-size:10px;margin-left:6px;" onclick="useSalesAsGross('${type.replace(/'/g, "\\'")}')">Use as Gross Sales</button></div>` : ''}
        </div>
      `;
      }).join('');
    }

    function useSalesAsGross(type) {
      const cs = computeSalesTotalsFor(type);
      const input = document.getElementById(`ns-${type}`);
      if (input) input.value = cs.total;
      showToast(`Filled Gross Sales for ${type} from ${cs.count} logged sale(s) — click "Save Gross Sales" to apply`, 'info');
    }
        ...
        <tbody>
          ${sales.map(s => `
            <tr>
              ...
              <td>${s.date}</td>
              ${hasSections ? `<td>${s.section ? `<span class="tag" style="color:var(--fuchsia-bright);border-color:var(--fuchsia-deep);">${s.section}</span>` : '—'}</td>` : ''}
    ...
    function renderSectionBreakdown(type) {
      ...
      grid.innerHTML = sections.map(sec => {
        const secRecords = records.filter(r => r.plantation_type === type && r.section === sec);
        const expenditure = round2(secRecords.reduce((s, r) => s + r.total_expenditure, 0));
        const yieldKg = round2(secRecords.reduce((s, r) => s + (r.yield_kg || 0), 0));
        const secSales = plantationSales.filter(s => s.plantation_type === type && s.section === sec);
        const salesTotal = round2(secSales.reduce((s, x) => s + x.total, 0));
        const salesQty = round2(secSales.reduce((s, x) => s + x.quantity, 0));
        const netForSection = round2(salesTotal - expenditure);
        return `
          <div class="plantation-total-card">
            <div class="p-name">📐 ${sec}</div>
            <div class="p-total">${peso(expenditure)}</div>
            <div class="p-meta">${secRecords.length} record(s) · yield ${yieldKg} kg</div>
            <div class="p-meta">🛒 ${secSales.length} sale(s) · ${salesQty} kg sold</div>
            <div class="p-profit ${netForSection >= 0 ? 'pos' : 'neg'}">Sales ${peso(salesTotal)} − Expenditure ${peso(expenditure)} = <strong>${peso(netForSection)}</strong></div>
          </div>
        `;
      }).join('');
    }
    ...
    function renderPlantationDetail(type) {
      ...
      if (recs.length === 0) {
        ...
        [...recs].sort((a,b) => b.date.localeCompare(a.date)).forEach(r => {
          tableHtml += `<tr><td class="num">#${r.id}</td><td>${r.date}</td>${hasSections ? `<td>${r.section ? `<span class="tag" style="color:var(--fuchsia-bright);border-color:var(--fuchsia-deep);">${r.section}</span>` : '—'}</td>` : ''}<td>${r.yield_kg ? r.yield_kg + ' kg' : '—'}</td><td class="num">${r.revenue ? peso(r.revenue) : '—'}</td><td class="num">${r.workers.length}</td><td class="num">${peso(r.total_expenditure)}</td></tr>`;
        });
      ...
      records.forEach(r => {
        ...
        if (haystack.includes(q)) {
          results.push({
            ...
            label: `#${r.id} · ${r.plantation_type}${r.section ? ' · ' + r.section : ''} · ${r.date}`,
